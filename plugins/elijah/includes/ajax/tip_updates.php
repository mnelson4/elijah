...
function elijah_strategy_update() {
	...
	if($start_or_skip=='start' || $start_or_skip == 'restart'){
		$result = Elijah_Tips_Applied_Logic::start_research_strategy($research_strategy_id,$current_research_goal_id);
	}else{
		$result = Elijah_Tips_Applied_Logic::skip_research_strategy($research_strategy_id, $current_research_goal_id);
	}
//	echo "result:$result";
...
function elijah_strategy_modified() {
	$connection_id = p2p_type('work_done')->get_p2p_id( $_POST['strategy-id'], $_POST['goal-id'] );
	...
	if( $_POST['strategy-usefulness'] >= ELIJAH_STRATEGY_USEFULNESS_FOUND_INFO ) {
		update_post_meta( $_POST['goal-id'],'research_status', 'resolved' );
	} else {
		//update to in progress or enqueued? do we need or want both?
		update_post_meta( $_POST['goal-id'], 'research_status', 'in-progress' );
	}
	die;
}